...
public class ViewMyMessagesActionTest extends TestCase {
	...
	public void testFilterMessagesEmpty() throws SQLException, ITrustException, ParseException {
		List<MessageBean> mbList = action2.getAllMyMessages();
		mbList = action2.filterMessages(mbList, ",,,,,");
		assertEquals(14, mbList.size());
	}

	public void testFilterMessagesName() throws SQLException, ITrustException, ParseException {
		List<MessageBean> mbList = action2.getAllMyMessages();
		mbList = action2.filterMessages(mbList, "Andy Programmer,,,,,");
		assertEquals(3, mbList.size());
		assertEquals("Andy Programmer",action2.getName(mbList.get(0).getFrom()));;
	}

	public void testFilterMessagesSubject() throws SQLException, ITrustException, ParseException {
		List<MessageBean> mbList = action2.getAllMyMessages();
		mbList = action2.filterMessages(mbList, ",Lab Results,,,,");
		assertEquals(2, mbList.size());
		assertEquals("Lab Results",mbList.get(0).getSubject());
	}

	public void testFilterMessagesHasWord() throws SQLException, ITrustException, ParseException {
		List<MessageBean> mbList = action2.getAllMyMessages();
		mbList = action2.filterMessages(mbList, ",,check,,,");
		assertEquals(3, mbList.size());
		assert(mbList.get(0).getBody().toLowerCase().contains("check".toLowerCase()));
	}
	
	public void testFilterMessagesNotHasWord() throws SQLException, ITrustException, ParseException {
        List<MessageBean> mbList = action2.getAllMyMessages();
        mbList = action2.filterMessages(mbList, ",,,medicine,,");
        assertEquals(12, mbList.size());
        assert(!mbList.get(0).getBody().toLowerCase().contains("check".toLowerCase()));
    }
    public void testFilterMessagesDate() throws SQLException, ITrustException, ParseException {
        List<MessageBean> mbList = action2.getAllMyMessages();
        mbList = action2.filterMessages(mbList, ",,,,01/01/2010,01/31/2010");
        assertEquals(9, mbList.size());
        DateFormat format = new SimpleDateFormat("MM/dd/yyyy");
        Date s = format.parse("01/01/2010");
        assert(s.before(mbList.get(0).getSentDate()));
    }
